...
import { GraduationCap, Mail, Lock, ArrowRight, Loader2, Github, Chrome, ShieldAlert, AlertCircle, Info, ExternalLink, UserPlus } from 'lucide-react';
    ...
    <div className="min-h-[calc(100vh-64px)] flex items-center justify-center p-4 bg-slate-50">
      <div className="max-w-md w-full bg-white rounded-3xl shadow-xl border border-slate-100 overflow-hidden animate-fade-in">
        <div className="bg-indigo-600 p-8 text-center text-white relative">
          ...
          <h1 className="text-2xl font-bold">EduVantage</h1>
          <p className="text-indigo-100 text-sm mt-1">Sua jornada rumo ao conhecimento</p>
        </div>
        ...
        <div className="p-8">
          ...
          <form onSubmit={handleAuth} className="space-y-4">
            ...
            <div className="space-y-1">
              ...
              <div className="relative">
                ...
                <input 
                  ...
                  className="w-full bg-slate-50 border border-slate-200 rounded-xl py-3 pl-12 pr-4 focus:ring-4 focus:ring-indigo-50 focus:border-indigo-500 outline-none transition-all"
                />
              </div>
            </div>
            ...
            <button 
              type="submit" 
              disabled={loading}
              className={`w-full text-white py-4 rounded-xl font-bold flex items-center justify-center gap-2 transition-all shadow-lg active:scale-95 disabled:opacity-70 ${isLogin ? 'bg-indigo-600 hover:bg-indigo-700 shadow-indigo-100' : 'bg-indigo-500 hover:bg-indigo-600 shadow-indigo-100'}`}
            >
          ...
          {email === 'wolf@wolf.com' && isLogin && (
            <div className="mt-4 p-3 bg-indigo-50 border border-indigo-100 rounded-lg text-indigo-600 text-[10px] font-bold uppercase text-center flex items-center justify-center gap-2">
              <AlertCircle size={14} />
              Credenciais Admin Sugeridas
            </div>
          )}
          ...
          <div className="mt-8">
            ...
            <div className="grid grid-cols-2 gap-4">
              <button className="flex items-center justify-center gap-2 border border-slate-200 py-3 rounded-xl hover:bg-slate-50 transition-colors text-sm font-medium">
